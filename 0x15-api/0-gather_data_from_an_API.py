...
def get_user(argv):
    ...
    tasksid = request.urlopen('https://jsonplaceholder.typicode.com/todos?userId={}'.format(argv))
    userid = request.urlopen('https://jsonplaceholder.typicode.com/users/{}'.format(argv))
    res = tasksid.read()
    user = userid.read()
    json_dct = json.loads(res.decode("utf-8"))
    json_usr = json.loads(user.decode("utf-8"))
    count = 0
    for index in range(0, len(json_dct)):
        if not json_dct[index]['completed']:
            count += 1
